...
# ── Role Matrix ──────────────────────────────────────────
# Laporan pengiriman  : admin, staff, keuangan
# Laporan invoice     : admin, keuangan  (staff TIDAK)
# Laporan pendapatan  : admin, keuangan  (staff TIDAK)
# ─────────────────────────────────────────────────────────
...
async def report_shipments(
    ...
    # Semua role boleh lihat laporan pengiriman
    ...
    # Staff hanya bisa lihat pengiriman yang dia buat sendiri
    if current_user["role"] == "staff":
        shipments = [s for s in shipments if s.get("created_by") == current_user["id"]]
...
async def report_invoices(
    ...
    if current_user["role"] == "staff":
        raise HTTPException(status_code=403, detail="Akses ditolak. Laporan invoice hanya untuk Admin dan Keuangan.")
...
async def report_revenue(
    ...
    if current_user["role"] == "staff":
        raise HTTPException(status_code=403, detail="Akses ditolak. Laporan pendapatan hanya untuk Admin dan Keuangan.")
